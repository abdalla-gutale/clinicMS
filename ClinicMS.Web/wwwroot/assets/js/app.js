...
(function () {
		...
		if (headerLangImg) {
			switch (lang) {
				...
				case "ch":
					...
					break;
				case "fr":
					headerLangImg.src = "/assets/images/flags/fr.svg";
					break;
				case "ar":
					headerLangImg.src = "/assets/images/flags/sa.svg";
					break;
				case "so":
					headerLangImg.src = "/assets/images/flags/so.svg";
					break;
				default:
			...
			if (this.readyState === 4 && this.status === 200) {
				try {
					...
					Array.from(placeholders).forEach(function (elem) {
						var placeholderKey = elem.getAttribute("data-placeholder-key");
						if (placeholderKey && data[placeholderKey]) {
							elem.setAttribute("placeholder", data[placeholderKey]);
						}
					});
				} catch (error) {
					console.error("Failed to parse language file:", language, error);
				}
			} else if (this.readyState === 4) {
				console.error("Failed to load language file:", language, this.status);
			}
		};
	...
	function pluginData() {
		...
		function initSelect2(scope) {
			...
			var selectElements = root.querySelectorAll("select:not([data-choices]):not([data-no-select2]):not(.select2-hidden-accessible)");
			...
			Array.from(selectElements).forEach(function (element) {
				...
				var isInModal = element.closest(".modal");

				$element.select2({
					width: "100%",
					placeholder: placeholder,
					allowClear: !element.required && !!placeholder,
					dropdownParent: isInModal ? window.jQuery(isInModal) : window.jQuery(document.body),
	...
	function pruneEmptySidebarParents() {
		...
		Array.from(navbarNav.children)
			.filter(function (item) {
				return item.classList && item.classList.contains("nav-item");
			})
			.forEach(function (item) {
				const menuLink = item.querySelector(":scope > a.menu-link");
				const submenu = item.querySelector(".collapse.menu-dropdown");
				const href = menuLink ? (menuLink.getAttribute("href") || "").trim() : "";

				if (!submenu) {
					if (menuLink && (href === "" || href === "#")) {
						item.remove();
					}
					return;
				}

				const childLinks = Array.from(submenu.querySelectorAll("ul.nav-sm > li.nav-item > a.nav-link"))
					.filter(function (link) {
						const parentItem = link.closest("li.nav-item");
						const parentStyle = parentItem ? window.getComputedStyle(parentItem) : null;
						const linkStyle = window.getComputedStyle(link);
						const hasText = link.textContent && link.textContent.trim().length > 0;

						return hasText &&
							(!parentItem || !parentItem.classList.contains("d-none")) &&
	...
	function toggleHamburgerMenu() {
		var windowSize = document.documentElement.clientWidth;
		var navLinks = document.querySelectorAll(".navbar-menu .navbar-nav .nav-link");

		var navbarMenu = document.querySelector(".navbar-menu");
		var navbarBrandBox = document.querySelector(".navbar-brand-box");
		var navbarBrandbox = document.querySelector("#navbarbrandbox");
		...
		// Hide before/after effects when menu is open
		if (document.body.classList.contains("menu-open")) {
			navLinks.forEach(link => {
				link.style.setProperty("--before-content", "none");
				link.style.setProperty("--after-content", "none");
				link.style.boxShadow = "none";
			});

			// Remove overflow:hidden
			if (navbarMenu) navbarMenu.style.overflow = "visible";
			if (navbarBrandBox) navbarBrandBox.style.overflow = "visible";
			if (navbarBrandbox) navbarBrandbox.style.overflow = "visible";
		} else {
			navLinks.forEach(link => {
				link.style.removeProperty("--before-content");
				link.style.removeProperty("--after-content");
				link.style.removeProperty("boxShadow");
			});

			// Add overflow:hidden to hide the content
			if (navbarMenu) navbarMenu.style.overflow = "hidden";
			if (navbarBrandBox) navbarBrandBox.style.overflow = "hidden";
			if (navbarBrandbox) navbarBrandbox.style.overflow = "hidden";
		}
